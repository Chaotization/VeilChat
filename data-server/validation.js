...
const exportedMethods = {
  ...
  checkGender(gender) {
    ...
    if (!validGenders.includes(gender))
      throw "Invalid gender provided. Gender must be either 'male', 'female', or 'others'.";
  ...
  checkAgeRange(age) {
    ...
    return {
      min: new Date(formatDate(minBirthYear)),
      max: new Date(formatDate(maxBirthYear)),
    };
  },
  ...
  generateCurrentDate() {
    ...
    return new Date(`${month}/${day}/${year} ${hour}:${minutes}:${seconds}`);
  },
  ...
  validateDateTime(inputDate) {
    const dateRegex =
      /^(0[1-9]|1[0-2])\/(0[1-9]|[12][0-9]|3[01])\/(19[0-9]{2}|20[0-9]{2})$/;
  ...
  validateGender(gender) {
    ...
    if (gender !== "male" && gender !== "female")
      throw "Invalid gender provided. Gender must be either 'male' or 'female'.";
    return gender;
  },
};
