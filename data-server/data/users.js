...
export const logoutUser = async (email, userId) => {
  const userCollection = await users();
  const user = await userCollection.findOne({
    email,
  });
...
export const updateFriendStatus = async (userId, friendId, newStatus) => {
  ...
  let exist =
    user.friends.hasOwnProperty(friendId) &&
    friend.friends.hasOwnProperty(userId);
  if (exist) {
...
export const createAccountWithEmailAndPassword = async (user) => {
  ...
  if(user.autoLogin)
  {
    await loginUser(user.email, social_password)
  }
  return insertUser
};
