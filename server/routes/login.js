...
router.route("/").post(async (req, res) => {
  ...
  try{
    ...
    const accessToken = jwt.sign({ userId }, process.env.JWT_SECRET="someSecret", { expiresIn: '60m' });
    req.session.user= user;
    req.session.loggedIn = true;   
    res.cookie('sessionToken', accessToken, { httpOnly: true });
      return  res.status(200).json({message:"success", loggedInFromServer:req.session.loggedIn});
    }
    else{
